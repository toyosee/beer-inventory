...
const {getBeers, createBeer, getBeer, updateBeer, deleteBeer, updateBeerStatus, getOrderedBeerById} = require('../controllers/beerController')

// Getting api routes for beer CRUD
router.route('/').get(getBeers).post(createBeer)
router.route('/ordered/:id').get(getOrderedBeerById);
