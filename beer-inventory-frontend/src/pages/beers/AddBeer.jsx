...
function AddBeer() {
  ...
  const sendEmailToStaff = async () => {
    try {

      // Construct the email body
      const body = `
        NOTIFICATION - New beer order:
        Beer Name: ${beer.name}
        Supplier ID: ${beer.supplier_id}
        Description: ${beer.description}
        Set aside all empty kegs from ${beer.supplier_id} Supplier for returns
      `;

      const recipientEmail = 'staff@example.com'; // Replace with the staff's email
      const subject = 'New Beer Order';
